module.exports = {
    ...
    get: function( req, res, next ) {
        ...
        if ( code ) {
            ...
            var sql2 = type == 1 ?
                'select ordermaster.*,comp_info.name,comp_info.shortname,count(orderdetail.oid) as itemcount ,SUM(orderdetail.good_amount) as total_amount  from comp_info,ordermaster left join orderdetail on orderdetail.order_id=ordermaster.order_id where ordermaster.order_status>-1 and comp_info.id=ordermaster.'+(req.session.role_type==2?'supplie_id':'comp_id')+' and '+(showHistory?'order_status!=5 and':'order_status=5 and ' )+' ordermaster.comp_id=' :
                'select ordermaster.*, comp_info.name,comp_info.shortname,count(orderdetail.oid) as itemcount ,SUM(orderdetail.good_amount) as total_amount  from comp_info,ordermaster left join orderdetail on orderdetail.order_id=ordermaster.order_id where ordermaster.order_status>-1 and comp_info.id=ordermaster.'+(req.session.role_type==2?'supplie_id':'comp_id')+' and '+(showHistory?'order_status!=5 and':'order_status=5 and ' )+' order_status!=1 and  ordermaster.supplie_id=';
